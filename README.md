...
- Declaring a new list or list iterators using:
  ```c
  List myList;
  ListIterator it;
- **`create():`**
...
- **`next():`**
  - Advances the iterator to the tail by 1 step.
	- Returns an iterator to the element if it's a valid element in the list, `NULL` otherwise
    ```c
...
- **`at():`**
  - Get an iterator pointing at the element in the specified index, including negative indexing.
	- `0` is the first element at the head, `-1` is the last element at the tail, `-2` is second to last, ...
	- The index of the element `[-size, size-1]` inclusive
...
- **`val():`**
  - Returns a pointer to the value in the node pointed to by the iterator.
    ```c
    ElementTypePtr value = val(currentIterator);
    ```
...
- **`addHead():`**
  - Used to allocate memory and add a new element of type `ElementType` form the head side of the list.
  - Returns a `ListIterator` to the added node or `NULL`  in case of failure.
    ```c
    ListIterator it = addHead(&myList, 12);

- **`addTail():`**
  - Used to allocate memory and add a new element of type `ElementType` form the tail side of the list.
  - Returns a `ListIterator` to the added node or `NULL`  in case of failure.
	  ```c
...
- **`fill():`**
  - Make all list elements equal to a value.
    ```c
    fill(&myList, 12);
    ```
...
- **`printAllElements():`**
  - Print all elements in the list in order from Head to Tail.
	- Caller must provide a separator of type string literal that will be printed between each two nodes.
    ```c
